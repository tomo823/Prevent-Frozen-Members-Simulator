...
class TopicVectorPipeline:
    ...
    def _preprocess_text(self, documents):
        """
        テキストデータから特定のノイズ単語を直接除去する。
        """
        print("   Preprocessing text: removing um, uh, so...")
        # 除去したい単語リスト
        noise_words = ["um", "uh", "so", "ah"]
        
        # 単語の境界(\b)を意識した正規表現パターンを作成
        # 例: r'\b(um|uh|so)\b' -> "summary"の"um"などは消さず、単独の"um"だけ消す
        pattern = re.compile(r'\b(' + '|'.join(noise_words) + r')\b', re.IGNORECASE)
        
        cleaned_docs = [pattern.sub('', doc) for doc in documents]
        return cleaned_docs
